...
const getExtraPackages = async (input) => {
  ...
  return data.objects.map((pkg) => ({
    name: `${pkg.package.name} \x1b[2m${pkg.package.description || ""}\x1b[0m`,
    value: pkg.package.name,
  }));
};

const GITHUB_REPO_URL =
  "https://github.com/harshgupta20/quickstart-react/issues";
...
(async () => {
  ...
  // 3. Create all necessary folder structure
  ...
  const config =
    routingConfig[routingFramework] || { folders: [], packages: [], devPackages: [] };
  ...
  // 10. Setup state management
  setupStateManagement(stateManagement, projectPath, isTS);
